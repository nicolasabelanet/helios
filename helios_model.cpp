...
void HeliosModel::createVertexBuffers(const std::vector<Vertex> &vertices) {
  ...
  heliosDevice.createBuffer(bufferSize, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                            vertexBuffer, vertexBufferMemory);

  void *data;
  vkMapMemory(heliosDevice.device(), vertexBufferMemory, 0, bufferSize, 0,
              &data);
  memcpy(data, vertices.data(), static_cast<size_t>(bufferSize));
  vkUnmapMemory(heliosDevice.device(), vertexBufferMemory);
}
